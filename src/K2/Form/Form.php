...
use K2\Form\Exception\FormException;
use K2\Validation\ValidationBuilder;
use K2\Form\Field\AbstractField as Field;
use K2\ActiveRecord\Validation\ValidationBuilder as ValidationAR;
...
class Form implements ArrayAccess, Validatable
{
    ...
    /**
     * 
     * @param string|\K2\Form\Field\Field $fieldName
     * @param string $type
     * @param array $options
     * @return \K2\Form\Field\Text|\K2\Form\Field\AbstractChoice
     * @throws Exception 
     */
    public function add($fieldName, $type = 'text', array $options = array())
    {
        if ($fieldName instanceof Field\Field) {
            ...
            $type = 'K2\\Form\\Field\\' . ucwords($type);
    ...
    public function render()
    {
        ...
        $string .= "</ul><ul>";
        foreach ($this->fields as $field) {
            if ($field instanceof \K2\Form\Field\Hidden) {
                continue;
            } elseif ($field instanceof \K2\Form\Field\Check ||
                    $field instanceof \K2\Form\Field\Radio) {
